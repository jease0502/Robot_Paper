...
Legged locomotion policies trained via deep reinforcement learning (RL) routinely
...
and often fragile process. Conversely, deploying post-hoc command smoothing on
...
loop. As illustrated in Fig.~\ref{fig:arch}, the policy emits joint position
...
\begin{figure*}[!tb]
\centering
\includegraphics[width=\textwidth]{figs/fig1-architecture.pdf}
...
In standardized training environments such as \texttt{unitree\_rl\_mjlab}, PD
gains are tuned for a target closed-loop natural frequency of $f_n = 10$\,Hz with
...
successive policy steps commands a peak transient
\begin{equation}
\tau_{\mathrm{step}} = k_p \cdot 2 \cdot \frac{0.25\,e}{k_p} = 0.5\,e .
\label{eq:step}
\end{equation}
...
junctions, current-based squared torque serves as the primary ground-truth
metric~\cite{urs}.

\section{Experimental Setup}

Four experimental protocols are evaluated. The labels follow the source
experiments; there is no E4, as the label was never assigned.

\textbf{E1 (single joint, synthetic signals).} A second-order joint model
representative of the Unitree Go1 ($k_p = 35$, damping 0.5, rotor inertia
...
model, AR(8), is fitted via least squares on baseline rollouts and rolled forward
two steps. This enables a symmetric Hann window to be applied without awaiting
...
\begin{figure}[!tb]
\centering
\includegraphics[width=\columnwidth]{figs/fig5-three-filters.pdf}
...
\begin{figure*}[!tb]
\centering
\includegraphics[width=\textwidth]{figs/e1-rho-sweep.png}
...
(Fig.~\ref{fig:crossover}). Consequently, posture demands dominate knee
dissipation, whereas jitter drives overheating in lightly loaded joints.

\begin{figure}[!tb]
\centering
\includegraphics[width=\columnwidth]{figs/fig2-crossover.pdf}
...
No \texttt{action\_rate} & 0.0723 & \textbf{2.53} ({+}8\%) & 3.43 & 0.119 \\
No gait terms         & 0.0747 & 2.62 ({+}12\%) & 4.98 & 0.146 \\
No torques + energy   & 0.1267 & \textbf{4.43} ({+}89\%) & 5.44 & 0.363 \\
...
\begin{figure*}[!tb]
\centering
\includegraphics[width=\textwidth]{figs/command-spectrum-annotated.png}
...
smoothing, serves as the primary regularizer of command-side thermal load.
...
\begin{tabular}{lrrc}
\toprule
Filtering scheme & $\mathcal{H}/\mathcal{H}_{\mathrm{ZOH}}$ & Preserved motion & Deployable \\
\midrule
...
Zero-phase Butterworth (12\,Hz) & 0.540 & \textbf{0.995} & No \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[!tb]
\centering
\includegraphics[width=\textwidth]{figs/e3-command-filters.png}
...
Zero-phase filtering achieves a 46\% thermal reduction while preserving 99.5\% of
intended trajectory motion. However, a causal implementation of the identical
...
inversely with joint holding load (Table~\ref{tab:joints}): lightly loaded hip
...
\begin{figure*}[!tb]
\centering
\includegraphics[width=\textwidth]{figs/fig3-band-budget.pdf}
...
evaporate due to phase lag (Table~\ref{tab:closedloop}).
...
Butterworth (12\,Hz) + lin. & 0.942 & 1.284 & $-0.26$ & 0.75 & 0.00 \\
Zero-phase, 40\,ms delay  & \textbf{1.082} & 2.493 & $-0.03$ & 1.00 & \textbf{0.07} \\
...
\begin{figure*}[!tb]
\centering
\includegraphics[width=\textwidth]{figs/e4-closed-loop.png}
...
Filtering the ZOH staircase harmonics does not drive overall thermal recovery
(Table~\ref{tab:nyquist}). Schemes that halve the $>25$\,Hz term generate higher
...
\begin{figure*}[!tb]
\centering
\includegraphics[width=\textwidth]{figs/fig4-latency-backfire.pdf}
...
increase (Table~\ref{tab:velocity}). Similarly, the 5\,Hz Butterworth induces
falls in 70\% of episodes at 1.0\,m/s. Gating the predictive filter by joint load
...
\begin{tabular}{lrr}
\toprule
Metric & Full reward & No effort terms \\
\midrule
Baseline ZOH heat & 255.9 & \textbf{376.3} ({+}47\%) \\
DC / gait / 5--25 / $>$25 (\%) & 31.3/50.6/16.5/1.6 & 27.0/29.7/40.7/2.6 \\
...
Predictive + gate tracking & 1.157 & 1.149 \\
Zero-phase delay $\mathcal{H}/\mathcal{H}_0$ & 1.082 & 1.053 \\
Zero-phase delay fall rate & 0.07 & \textbf{0.37} \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure*}[!tb]
\centering
\includegraphics[width=\textwidth]{figs/e4-closed-loop-no-torques-energy.png}
...
\section{Practical Engineering Guidelines}

\textbf{Profile spectral splits before intervening.} A single baseline rollout
establishes the reachable budget. Halving this reachable share provides an a
priori estimate of closed-loop thermal recovery without modifying low-level code.
If the reachable fraction is small, efforts must target kinematic posture or reward
formulations rather than filtering.

\textbf{Avoid delayed zero-phase smoothing.} Smoothing latency directly erodes
feedback phase margin. Candidate smoothing architectures must be validated at
maximum commanded velocities, where latency-induced falls manifest.

\textbf{Deploy load-dependent gating.} Restrict command smoothing to joints
operating below $\bar{\tau} \approx \sqrt{\mathrm{Var}(\tau)}$. This preserves
tracking fidelity on stance joints while eliminating jitter on unloaded joints.

\textbf{Prioritize effort regularization over action rate.} Reward terms
penalizing joint torque and power dictate 89\% of transient thermal load, compared
to 8\% governed by discrete finite-difference \texttt{action\_rate} penalties.

\section{Related Work}

\textbf{Thermal-aware locomotion.} Recent frameworks integrate motor temperature
observations and thermal reward boundaries~\cite{thermalaware}, train thermal
residual policies~\cite{residual}, or employ contact-constrained inverse kinematics
for multi-limbed humanoid thermal balancing~\cite{jorgensen}. These approaches
require complete retraining, whereas this work establishes post-hoc interventions
on frozen policies.

\textbf{Actuation interfaces.} Interfaces bridging high-level commands and motor
drives include reality shaping via two-degree-of-freedom tracking
controllers~\cite{yamamori}, high-frequency torque interpolation~\cite{kourdis},
voltage-realizable acceleration bounds~\cite{vra}, action chunk
post-optimization~\cite{lipo}, B-spline policy representations~\cite{bspline}, and
forward delay compensation networks~\cite{weddington}. We characterize the
energetic dissipation consequences of these interfaces.

\textbf{Smoothness regularization.} Methods such as CAPS~\cite{caps},
Lipschitz-constrained architectures~\cite{lipschitz}, and spectral
normalization~\cite{specnorm} suppress command oscillations during training, and a
benchmark study reports improved smoothness at small performance
cost~\cite{benchmark}. However, these studies do not isolate physical copper loss
from steady-state posture loads.

\textbf{Control discretization.} Low-frequency execution has demonstrated
robustness on ANYmal C at 8\,Hz~\cite{gangapurwala}, Q-learning degenerates as the
time step shrinks~\cite{tallec}, and adaptive action-duration frameworks reduce
average control frequency~\cite{tarc}. None of these works map electrical
dissipation as an explicit function of control rate.

\section{Limitations and Future Work}

This study relies on rigid-body simulation using squared torque as a proxy for
electrical winding loss under constant torque sensitivity $K_t$. Stator iron core
losses, inverter switching dynamics, and winding thermal diffusion are omitted.
Single-joint analytical sweeps omit ground impact dynamics. Evaluations are
conducted on flat terrain using a single quadruped morphology, the Unitree Go1. The
5\,Hz gait split is hand-chosen and has not been swept, and the reward ablations
are single-seed over 8\,s intervals with no confidence intervals reported.
Extending four-band spectral decomposition to humanoid platforms such as the
Unitree G1, and validating the $3.2\Nm$ crossover on physical dynamometer
testbenches, represent essential future directions.
...
GR00T-WholeBodyControl, GitHub issue \#174, 2026. [Online]. Available:
...
\texttt{example\_py/example\_position.py}, GitHub, 2024. [Online]. Available:
...
company blog, 2025. [Online]. Available: \url{https://www.deeprobotics.us/news/}

\bibitem{gom8010}
Unitree Robotics, ``GO-M8010-6 motor data user manual,'' V1.0, 2023.

\bibitem{j60}
DEEP Robotics, ``J60 joint,'' product page, 2024. [Online]. Available:
...
recovery of multi-limbed robots with electric actuators,'' \emph{IEEE Robot.
