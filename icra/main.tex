...
\item \textbf{Regularization diagnostics.} We demonstrate that conventional
\texttt{action\_rate} penalties exert minimal authority over command-side thermal
load ($+8\%$ step transient when ablated), whereas direct effort penalties dictate
the accessible thermal budget ($+89\%$).
\end{itemize}
...
\begin{table}[!tb]
\caption{Command-side thermal statistics across reward ablations (E2), five seeds.
$\tau_{\mathrm{step}}$ is mean $\pm$ s.d.\ across seeds; the last column is the
change against that seed's own baseline, averaged. $^{\textrm{ns}}$ marks an
interval that straddles zero. The spectral centroid and
$\rho_{>5\,\mathrm{Hz}}$ are omitted here for column width and are reported in
...
one seed of five the ablated policy is \emph{smoother} than its own baseline. An
earlier single-seed draft reported ${+}8\%$; that was one draw from this
distribution. Conversely, eliminating joint torque and energy regularizations
...
pp. 211--216, 2025, arXiv:2509.12739.
...
Datasets Benchmarks Track}, 2021, arXiv:2108.10470.
...
Autom. Syst.}, vol. 23, no. 11, pp. 3284--3292, 2025, arXiv:2506.05165.
